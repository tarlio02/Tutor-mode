...
- **Live whiteboard drawing** — the tutor writes headings, equations, diagrams, and graphs stroke-by-stroke with an animated pen, on an endless scrolling white "page" that pans downward as the lesson progresses
- **Real math symbols** — equations render with actual glyphs (π, β, Δ, ∫, √, ≤, →, …) instead of spelled-out words, in a font stack chosen for good symbol coverage
- **Neat, labeled graphs** — plots get arrowed axes, faint gridlines, optional tick labels, and an optional title by default, so every graph is easy to read at a glance
- **A fuller color palette** — beyond amber/sage/brick, the tutor can use teal, violet, sky, and coral to distinguish multiple curves or terms, plus a soft "halo" glow it can place behind a key result to make it pop
